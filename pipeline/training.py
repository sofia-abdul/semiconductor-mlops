...
def _save_evaluation_figures(model, X_test, y_test, threshold, model_name):
    """Generate and save all evaluation figures from the held-out test set."""
    ...
    ax.set_title("Target class distribution — test set")
    ax.set_ylabel("Count")
    ...
    ax.set_title(
        ...
        f"threshold = {threshold:.4f}  |  test set (n={len(y_test)})"
    )
    plt.tight_layout()
    ...
    p_at_t = precision_score(y_test, preds, zero_division=0)
    r_at_t = recall_score(y_test, preds, zero_division=0)
    ax.plot(
        r_at_t,
        p_at_t,
        "ro",
        ...
        zorder=5,
        label=(
            f"Operating point  t={threshold:.2f}\n"
            f"P={p_at_t:.3f}  R={r_at_t:.3f}"
        ),
    )
 
    # Naive baseline: always predict positive
    naive_precision = y_test.mean()
    ax.axhline(
        naive_precision,
        color="gray",
        linestyle="--",
        linewidth=0.8,
        label=f"Naive baseline  P={naive_precision:.3f}",
    )
    ...
    ax.spines["right"].set_visible(False)
    plt.tight_layout()
    plt.savefig(FIG_DIR / "precision_recall_curve.png", dpi=300)
    plt.close()
    print("Saved: precision_recall_curve.png")
 
   
    # 5. Feature importance top 20
    importance_csv = Path(FEATURE_IMPORTANCE_PATH)
    if importance_csv.exists():
        imp_df = pd.read_csv(importance_csv)
        imp_col = [c for c in imp_df.columns if c != "feature"][0]
        imp_df = (
            imp_df.sort_values(imp_col, ascending=False)
            .head(20)
            .sort_values(imp_col, ascending=True)  
        )
        fig, ax = plt.subplots(figsize=(7, 6))
        ax.barh(
            imp_df["feature"].astype(str),
            imp_df[imp_col],
            color="#378ADD",
            edgecolor="white",
        )
        ...
        ax.spines["right"].set_visible(False)
        plt.tight_layout()
        plt.savefig(FIG_DIR / "feature_importance_top20.png", dpi=300)
        plt.close()
        print("Saved: feature_importance_top20.png")
    else:
        print(
            f"Skipping feature importance figure — "
            f"CSV not found at {importance_csv}"
        )
...
def train_models(X_train, X_test, y_train, y_test) -> pd.DataFrame:
    ...
    # save_feature_importance must run before _save_evaluation_figures
    # so the feature importance CSV exists when the figure function reads it
